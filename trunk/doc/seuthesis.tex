...
{\color{magenta}
\begin{verbatim}
\documentclass[<class_options>]{seuthesis}
\end{verbatim}
}
该命令使用\seuthesis 文档类，其中\verb+<class_options>+用\texttt{masters, phd, engieering} 来分别表示
...
如果用户还需要载入更多的宏包，可以通过这个命令载入。但是前提是不能破坏\seuthesis 文档类的基本参数设定。
...
该命令自动生成参考文献，使用\texttt{seuthesis.bst} 指定的文献风格。
...
\section{参考文献格式}
见\verb+seuthesis.bst+的文档: 本文第\ref{bst}章。
...
\begin{figure}
\centering
\caption{\seuthesis logo\label{logo}}
\chuhao \seuthesis
\end{figure}

\section{算法格式处理}
\seuthesis 文档类采用了\texttt{algorithm, algorithmic }两个宏包来设置算法排版格式。
详细使用方法参见这两个宏包的手册。这里给出一个简单的例子。
{\color{magenta}
\begin{verbatim}
\begin{algorithm}
\caption{\label{algoinsight}如何使用\seuthesis 文档类}
\begin{algorithmic}[1]
\STATE 具备一定的\LaTeX 使用经验
\STATE 有耐心阅读文档
\STATE 仔细阅读本文档
\STATE 在使用中熟悉它
\end{algorithmic}
...
至少两次。
\chapter{\texttt{seuthesis.bst}参考文献格式\label{bst}}
\section{简介}
\texttt{seuthesis.bst} 是符合东南大学硕士和博士毕业论文参考文献格式要求的 bibliography style。
由于该文献格式是数字标签(numeric label)格式，且无需排序。因此，它是在标准的BibTeX
格式\texttt{unsrt.bst} 的基础上发展而来。同时，\verb=seuthesis.bst=文献格式不支持\texttt{crossref}字段。
...
\texttt{seuthesis.bst}支持以下几种不同的entry type。
\begin{description}
\item[book] 书籍
\item[article] 期刊文章
...
\chapter{全文总结}
本文主要介绍了如何使用\seuthesis \LaTeX 文档类来对东南大学硕士与博士学位论文进行排版。
当然，\seuthesis 肯定存在许多不足的地方，希望各位读者予以指正。

\acknowledgement
感谢每一个支持\seuthesis 的人。
...
\resume{作者简介}
\seuthesis 开发组，目前仅有四人(当前活跃用户只有樊智猛一人)，希望感兴趣的同学加入。
